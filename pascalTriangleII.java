...
// 所以我们需要上一行的数据就很自然的可以去取。而这里我们只需要一行数据，
// 就得考虑一下是不是能只用一行的空间来存储结果而不需要额外的来存储上一行呢？
...
// 下一步就不需要了。这个技巧在动态规划省空间时也经常使用，主要就是看我们需要的数据是原来的数据还是新的数据来决定我们遍历的方向。
...
public ArrayList<Integer> getRow(int rowIndex) {  
    ...
    {  
        ...
        {  
            res.set(j+1,res.get(j)+res.get(j+1));  //Arrayset.set(index, elem)
        }  
        res.add(1);  
    }  
    return res;  
